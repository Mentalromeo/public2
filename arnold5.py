...
author = 'MsrXx'
git_hub = 'github.com/MENTALROMEO'
faceb0ok = 'ARNOLD HARDY'
...
try:
	clear_layar()
	print(logo2())
	print(f'\r\n [{hh}>{P}] currently dumping proxy and create useragent by arnold')
...
except:sys.exit(f" [{M}>{P}] uff Arnold failed proxy dump")
...
def login():
	clear_layar()
	print(logo2())
	cookie = input(f"\n [{hh}<{P}] don't use personal account\n cookie : ")
...
def menu(n,t,c):
	...
	print(f" [{hh}02{P}] CRACK MULTY 🌝   [{hh}08{P}] CRACK FROM FILE 🌝")
...
def cek_opsi_cp():
	...
	for data in open('CP/'+file,'r').read().splitlines():
		...
		cek_opsi(id,pw,ua)
	exit(f'\r [{hh}<{P}] Arnold check the checkpoint option has been completed')
...
def cek_akun():
	...
	except:sys.exit(f" [{M}>{P}] no ok result")
	for x in ok:
		nom.append(x)
		no+=1
	...
	exc = input(f' [{hh}<{P}] arnold file number to check\n file : ')
	xxx = input(' save file name  : \n nama : ')
	nonon = xxx+'.txt'
	file = nom[int(exc)-1]
	print(' ─────────────────────────────')
	print(f' account that is disabled at : {non-non}\n account that is disabled at: remove the idiot')
	print(' ─────────────────────────────')
	try:
...
def cek_hasil():
	no,nom = 0,[]
	one = input(f' [{hh}1{P}] check result  ok\n [{hh}2{P}] check result cp\n menu : ')
	if one in ['1','01']:
		try:ok = os.listdir('OK')
		except:sys.exit(f" [{M}>{P}] no ok result")
		for x in ok:
		...
		except:sys.exit(f" [{M}>{P}] file no resultsl ok")
		print(hh+buka+P)
	elif one in ['2','02']:
		try:ok = os.listdir('CP')
...
def manual():
	...
	for x in B:
		pwx.append(x)
	print(f"{P} ─────────────────────────────")
	print(f' akun ok di : {ARNOLD_ok}\n akun cp di : {ARNOLD_cp}')
	print(f"{P} ─────────────────────────────")
	awal = datetime.now()
